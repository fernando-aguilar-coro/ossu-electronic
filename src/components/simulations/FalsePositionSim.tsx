...
interface FalsePositionSimProps {
  defaultExpr?: string;
  defaultXa?: string;
  defaultXb?: string;
  defaultEs?: string;
...
export default function FalsePositionSim({
  defaultExpr = 'exp(-x) - x',
  defaultXa = '0',
  defaultXb = '1',
  defaultEs = '1',
  defaultImax = '20',
}: FalsePositionSimProps) {
  ...
  const onDrawOverlay = useCallback((
    ...
    if (isFinite(fxa_val) && isFinite(fxb_val)) {
      const cx_a = toX(cur.xa);
      const cy_a = toY(fxa_val);
      const cx_b = toX(cur.xb);
      const cy_b = toY(fxb_val);

      ctx.beginPath();
      ctx.strokeStyle = isDark ? 'rgba(245,158,11,0.6)' : 'rgba(217,119,6,0.6)';
      ctx.lineWidth = 2;
      ctx.moveTo(cx_a, cy_a);
      ctx.lineTo(cx_b, cy_b);
      ctx.stroke();
    }
        ...
        <Card>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, flexWrap: 'wrap', gap: 6 }}>
            <span style={{ fontWeight: 700, fontSize: 15, color: 'var(--ifm-font-color-base)' }}>
              Representación Geométrica (Falsa Posición)
            </span>
            <span>
